...
function activateUpgradeAbility() {
    ...
    if (isRemoveAbilityActive) {
        isRemoveAbilityActive = false;
        if (gameCanvas) gameCanvas.classList.remove('remove-cursor-active');
        // Ensure the remove button's visual state is also reset if you have one (e.g., border)
    ...
    if (isUpgradeAbilityActive) {
        ...
        canDrop = false; // Prevent normal dropping while ability is active
        // Add visual feedback to the button itself (e.g., border)
...
const buttonsWithClickSound = [
    ...
    // Add other button IDs or use a class selector like document.querySelectorAll('.clickable-button')
];
...
function handleFruitClick(event) {
    ...
    if (clickedBody) {
        // Check if the fruit is static (not fallen yet)
...
function startGame() {
    ...
    document.body.addEventListener('touchmove', function(e) {
        if (e.target === gameCanvas || e.target === gameContainer) {
            e.preventDefault();
        }
    }, { passive: false });
...
function activateUpgradeAbility() {
    ...
    if (isUpgradeAbilityActive) {
        console.log('Upgrade ability activated');
        canDrop = false;
        if (abilityUpgradeIcon) abilityUpgradeIcon.style.border = '2px solid yellow';
    } else {
        console.log('Upgrade ability deactivated');
...
// Function to toggle remove ability
function toggleRemoveAbility() {
    if (isRemoveAbilityActive) {
        isRemoveAbilityActive = false;
        if (abilityRemoveIcon) abilityRemoveIcon.style.border = 'none';
